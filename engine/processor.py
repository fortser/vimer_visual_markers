...
APPLY_ORDER: list[str] = [
    ...
    "5.1", "5.2",  # кавычки
    # 3.4 перенесён сюда: слипание тире должно применяться ДО замены
    # символа тире маркерами 5.3/5.4/5.5, иначе 3.4 не находит «—»
    "3.4",
    "5.3", "5.4", "5.5",  # тире → замены (после 3.4)
    ...
    # 3. Пропущенные пробелы (3.4 перенесён выше в блок тире)
    "3.1", "3.2", "3.3",
    # 2. Лишние пробелы
